...
####Get Data
CONF_URL = 'time_series_covid19_confirmed_global.csv'
DEAD_URL = 'time_series_covid19_deaths_global.csv'
RECV_URL = 'time_series_covid19_recovered_global.csv'
...
##get overall worldwide total for confirmed,recovered and dead cases
...
df = process_data(data=covid_conf_ts, cntry=cntry, window=3)
df.head(10)
##fig=px.line(df,x=df.index,y=df.Total)
fig = px.line(df, y="Total", x=df.index, title="Daily confirmed cases trend for {}", height=600,
              color_discrete_sequence=['maroon'])
fig.update_layout(title_x=0.5, plot_bgcolor='#F2DFCE', paper_bgcolor='#F2DFCE', xaxis_title="Date", yaxis_title="")
...
def fig_world_trend(cntry='US', window=3):
    ...
    fig = px.line(df, y="Total", x=df.index, title="Daily confirmed cases trend for {}".format(cntry), height=600,
                  color_discrete_sequence=['maroon'])
...
def get_country_list():
    unique_list = covid_conf_ts['Country/Region'].unique()
    ##data = unique_list[1:3]
    return unique_list
...
def get_country_dropdown(id):
    return html.Div([
        ...
        dcc.Dropdown(id='my-id' + str(id),
                     options=[{'label': x, 'value': x} for x in create_dropdown_list(get_country_list())],
                     value='US'
                     ),
...
def generate_cards(cntry='US'):
    ...
    cards = html.Div(
        [
            dbc.Row(
                [
                    dbc.Col(dbc.Card(generate_card_content("Recovered", recv_cntry_total, recv_overall_total),
                                     color="success", inverse=True), md=dict(size=3, offset=2)),
                    dbc.Col(dbc.Card(generate_card_content("Confirmed", conf_cntry_total, conf_overall_total),
                                     color="warning", inverse=True), md=dict(size=3)),
                    dbc.Col(dbc.Card(generate_card_content("Dead", dead_cntry_total, dead_overall_total), color="dark",
                                     inverse=True), md=dict(size=3)),
        ...
        [
            dbc.Col(
                ...
                dcc.Dropdown(id='my-id1',
                             options=[{'label': x, 'value': x} for x in get_country_list()],
                             value='US')], md=dict(size=5, offset=4)
